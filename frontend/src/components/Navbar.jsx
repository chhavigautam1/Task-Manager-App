...
const Navbar = ({user={},onLogout}) => {
  ...
  return (
    <header className="sticky top-0 z-50 bg-white/90 backdrop-blur-mid shadow-sm border-b border-gray-200 font-sans">
      <div className="flex items-center justify-between px-4 py-3 md:px-6 max-w-7xl mx-auto">
        {/* LOGO */}
        <div
          className="flex items-center gap2 cursor-pointer group"
          onClick={() => navigate("/")}
        >
          <div className="relative w-10 h-10 flex items-center justify-center rounded-xl bg-gradient-to-br from-fuchsia-500 via-purple-500 to-indigo-500 shadow-lg group-hover:shadow-purple-300/50 group-hover:scale-105 transition-all duration-300">
            ...
            <div className="absolute -bottom-1 -middle-1 w-3 h-3 ng-white rounded-full shadow-md animate-ping" />
          </div>
          {/* BRAND NAME */}
          <span className="ml-2 text-2xl font-extrabold bg-gradient-to-r from-fuchsia-500 via-purple-500 to-indigo-500 bg-clip-text text-transparent tracking-wide">
            Task Manager
          </span>
        </div>
        {/* right side */}
        <div className="flex items-center gap-4">
          ...
          <div ref={menuref} className="relative">
            <button
              ...
              className="flex items-center gap-2 px-3 py-2 rounded-ful cursor-pointer hover:bg-purple-50 transition-colors duration-300 border-transparent hover:border-purple-200"
            >
              ...
              <div className="text-left hidden md:block">
                ...
                <p className="text-xm text-gray-500 font-normal">
                  {user.email}
                </p>
              </div>
              <ChevronDown
                className={`w-4h-4 text-gray-500 transition-transform duration-300 ${
                  menuOpen ? "rotate-180" : ""
                }`}
              />
            </button>
